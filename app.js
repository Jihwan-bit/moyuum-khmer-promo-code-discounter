...
async function loadData(){
  ...
  state.products = state.products.map(p => {
    ...
    const rawAll = String(p.Image1||"") + " " + String(p.Image2||"");
...
function renderProducts(){
  ...
  for (const p of list){
    ...
    img.onerror = () => { imgWrap.innerHTML = "<div style='color:rgba(255,255,255,.55);font-size:12px'>No image</div>"; };
    ...
    meta.innerHTML = `
      ...
      <span>${escapeHtml(String(p["Main/Acc. Item"] || ""))}</span>
    `;
...
function computePromoGiftOffers(){
  ...
  // - TRIGGER rows (Convention B): the row barcode is the trigger; Promotion Barcode lists gift candidates.
  // - GIFT rows (Convention A): Promotion Barcode lists triggers; the row itself is the gift candidate.
  for (const row of products){
    if (String(row['Promotion Type'] || '').trim().toLowerCase() !== 'free') continue;
    const promoQty = Number(row['Promotion Quantity'] || 0);
    const freeQty  = Number(row['Free Quantity'] || 0);
    const rowBarcode = normalizeBarcode(row._barcode_norm || row.Barcode);
    const listBarcodes = parseBarcodeList(row['Promotion Barcode']);
    if (!(promoQty > 0 && freeQty > 0)) continue;
    if (!rowBarcode || !listBarcodes.length) continue;

    // Count MAIN vs ACC targets referenced by listBarcodes
    let mainCount = 0;
    let accCount  = 0;
    for (const b of listBarcodes){
      const p = byBarcode.get(normalizeBarcode(b));
      if (!p) continue;
      if (p._is_main) mainCount += 1;
      else accCount += 1;
    }
    ...
    let role = '';
    if (row._is_main){
      if (accCount > 0 && mainCount === 0) role = 'trigger';
      else if (mainCount > 0 && accCount === 0) role = 'gift';
      else role = (accCount >= mainCount) ? 'trigger' : 'gift';
    }else{
      if (mainCount > 0 && accCount === 0) role = 'gift';
      else if (accCount > 0 && mainCount === 0) role = 'trigger';
      else role = (mainCount >= accCount) ? 'gift' : 'trigger';
    }

    // HARD BLOCK: ACC items cannot be triggers (prevents "nipple triggers bottle")
    if (role === 'trigger' && !row._is_main) continue;
    ...
    if (role === 'trigger'){
      ...
      const candKey = listBarcodes
        ...
        .sort()
        .join('|');
      const catKey = String(row.Category || '').trim();
      const pooledKey = `B_POOL__CAT_${catKey}__CANDS_${candKey}__PQ${promoQty}__FQ${freeQty}`;
      ...
      // Convention A
      addOffer({
        triggerBarcodes: listBarcodes,
        promoQty,
        freeQty,
        triggerProduct: byBarcode.get(normalizeBarcode(listBarcodes[0])) || null,
        candidateProducts: [row],
        forceIsGroup: (listBarcodes.length > 1)
      });
    }
  }
...
function renderCart(){
  ...
  for (const {barcode, qty} of entries){
    ...
    left.innerHTML = `
      ...
      <div class="sub">${escapeHtml(categoryLabel(String(p.Category || "")))} · ${escapeHtml(String(p["Main/Acc. Item"] || ""))}</div>
    `;
